<template>
  <div v-if="this.callNumber && this.callNumber.length > 0" class="call-number-underway">
    <template>
      <el-table :data="callNumber" style="width: 100%;" :row-class-name="this.renderState">
        ...
        <el-table-column label="调度人" width="180">
          <template slot-scope="scope">
            <i class="el-icon-time"></i>
            <span style="margin-left: 10px">{{ scope.row.date }}</span>
          </template>
        </el-table-column>
        <el-table-column label="视频开始时间" width="180">
          <template slot-scope="scope">
            <i class="el-icon-time"></i>
            <span style="margin-left: 10px">{{ scope.row.date }}</span>
          </template>
        </el-table-column>
        <el-table-column label="视频结束时间" width="180">
          <template slot-scope="scope">
            <i class="el-icon-time"></i>
            <span style="margin-left: 10px">{{ scope.row.date }}</span>
          </template>
        ...
        <el-table-column label="操作">
          <template slot-scope="scope">
            <el-button size="mini" @click="handleEdit(scope.$index, scope.row)">重新呼叫</el-button>
          </template>
        </el-table-column>
      </el-table>
    </template>
...
@Component({})
class CallNumberUnderway extends Vue {
  @Prop({ default: null })
...
export default CallNumberUnderway;
</script>

<style lang="scss" scoped>
.call-number-underway {
  margin: auto;
  display: flex;
  .el-table {
    .warning-row {
